...
print(as.matrix(euclidienne), type = "html")
#print(xtable(as.matrix(round(dist(tabtp[,c("Longueur","Largeur","Poids")]),2))),type='html')
```
...
## Resultats 
```{r exempledendrogramme ,echo=FALSE, tidy=TRUE, warning=FALSE,error=FALSE,message=FALSE,out.width=".5\\linewidth"}
tabtp <- read.delim("donnees/aveclesmains.txt", dec=",")
euclidienne<-dist(tabtp[,c("Longueur","Largeur","Poids")])
library(cluster)
dd<-as.hclust(agnes(euclidienne,method="single"))
plot(dd, main = "Resultat de la classification",xlab = "individus", sub = "",hang=-1)
...
```{r inconvenientsward,echo=FALSE, tidy=TRUE, warning=FALSE,error=FALSE,message=FALSE,out.width=".5\\linewidth"}
set.seed(665544)
...
top<-cutree(agnes(x,method="ward"),4)

plot(x,xlim=c(-1,21),ylim=c(0,21),xlab="x",ylab="y",asp=1,type="n",main=" Classification de Ward")
for (i in 1:4)
{
points(x[which(top==i),],pch=paste(i),col=i)
}
...
```{r distanceeuclidienne22 ,echo=FALSE, results='asis', warning=FALSE,error=FALSE}
tabtp <- read.delim("donnees/aveclesmains.txt", dec=",")
euclidienne<-dist(tabtp[,c("Longueur","Largeur","Poids")])
print(euclidienne)
#print(xtable(as.data.frame(round(euclidienne,2))),type="html")
```
